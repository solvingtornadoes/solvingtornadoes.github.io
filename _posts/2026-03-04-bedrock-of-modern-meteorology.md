...
1. **Radiosonde Data (Weather Balloons):** Millions of balloons have tracked temperature and humidity profiles. They consistently show that when the atmosphere is "unstable" (as defined by the convection model), storms are more likely to form.  
2. **Laboratory Cloud Chambers:** We can verify the release of latent heat during condensation in a vacuum or chamber.  
3. **Numerical Modeling:** Computer simulations (like the WRF model) use the convection equations to predict weather. The fact that these models can predict the *general* location and timing of storms is used as "proof" that the underlying physics are correct.
